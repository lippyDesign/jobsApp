	...
	renderCards() {
		...
		const deck = this.props.data.map((item, i) => {
			...
			if (i === this.state.index) {
				return <Animated.View key={item[this.props.keyProp]} { ...this.state.panResponder.panHandlers } style={[this.getCardStyle(), styles.cardStyle]}>
			...
			return <Animated.View key={item[this.props.keyProp]} style={[styles.cardStyle, { top: 10 * (i - this.state.index), zIndex: -i }]}>{this.props.renderCard(item)}</Animated.View>
		});
