...
class MapScreen extends Component {
  ...
  render() {
    if (!this.state.mapLoaded) return <View style={{ flex: 1, justifyContent: 'center' }}><ActivityIndicator size='large' /></View>;
    return <View style={{ flex: 1}}>
      <MapView style={{ flex: 1}} region={this.state.region} onRegionChangeComplete={this.onRegionChangeComplete}/>
      <View style={styles.buttonContainer}>
        <Button large title='Search Jobs' backgroundColor='#009688' icon={{ name: 'search' }} onPress={this.onButtonPress} />
      </View>
    </View>;
  }
}
